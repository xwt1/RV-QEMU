                                            ...
                                            const void *v_tb_ptr)
{
    ...
    for (;;) {
        ...
        CASE_32_64(ld8u)
            ...
            regs[r0] = *(uint8_t *)ptr;
            // xwt
            printf("8bit load%u",*(uint8_t *)ptr);
            break;
        CASE_32_64(ld8s)
            tci_args_rrs(insn, &r0, &r1, &ofs);
            ptr = (void *)(regs[r1] + ofs);
            regs[r0] = *(int8_t *)ptr;
            printf("8bit load%d",*(int8_t *)ptr);
            break;
        CASE_32_64(ld16u)
            tci_args_rrs(insn, &r0, &r1, &ofs);
            ptr = (void *)(regs[r1] + ofs);
            regs[r0] = *(uint16_t *)ptr;
            printf("16bit load%u",*(uint16_t *)ptr);
            break;
        CASE_32_64(ld16s)
            tci_args_rrs(insn, &r0, &r1, &ofs);
            ptr = (void *)(regs[r1] + ofs);
            regs[r0] = *(int16_t *)ptr;
            printf("16bit load%d",*(int16_t *)ptr);
            break;
        ...
        CASE_64(ld32u)
            ...
            printf("32bit load%u",*(uint32_t *)ptr);
            break;
        CASE_32_64(st8)
            tci_args_rrs(insn, &r0, &r1, &ofs);
            ptr = (void *)(regs[r1] + ofs);
            *(uint8_t *)ptr = regs[r0];
            printf("8bit store%u",*(uint8_t *)ptr);
            break;
        CASE_32_64(st16)
            tci_args_rrs(insn, &r0, &r1, &ofs);
            ptr = (void *)(regs[r1] + ofs);
            *(uint16_t *)ptr = regs[r0];
            printf("16bit store%u",*(uint16_t *)ptr);
            break;
        ...
        CASE_64(st32)
            ...
            *(uint32_t *)ptr = regs[r0];
            printf("32bit store%u",*(uint32_t *)ptr);
            break;
    ...
    TCG_TARGET_HAS_bswap16_i32 || TCG_TARGET_HAS_bswap16_i64
        ...
        case INDEX_op_ld32s_i64:
            ...
            regs[r0] = *(int32_t *)ptr;
            // xwt
            printf("64bit load%d",*(int32_t *)ptr);
            break;
        case INDEX_op_ld_i64:
            tci_args_rrs(insn, &r0, &r1, &ofs);
            ptr = (void *)(regs[r1] + ofs);
            regs[r0] = *(uint64_t *)ptr;
            printf("64bit load%llu",(unsigned long long)*(uint64_t *)ptr);
            break;
        case INDEX_op_st_i64:
            tci_args_rrs(insn, &r0, &r1, &ofs);
            ptr = (void *)(regs[r1] + ofs);
            *(uint64_t *)ptr = regs[r0];
            printf("64bit store%llu",(unsigned long long)*(uint64_t *)ptr);
            break;
